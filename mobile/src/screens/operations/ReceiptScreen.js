...
export default function ReceiptScreen() {
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={s.content}>
      <Text style={s.title}>Incoming goods</Text>
      <Text style={s.label}>Supplier / Reference</Text>
      ...
      <Text style={s.label}>Products received</Text>
      {moves.map((move, i) => (
          ...
          <View style={s.selectWrap}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {products.slice(0, 50).map((p) => (
                <TouchableOpacity
                  key={p.id}
                  style={[s.optionChip, move.product_id === String(p.id) && s.optionChipActive]}
                  onPress={() => updateLine(i, 'product_id', String(p.id))}
                >
      ...
      <TouchableOpacity style={s.button} onPress={submit} disabled={loading}>
        {loading ? <ActivityIndicator color="#fff" /> : <Text style={s.buttonText}>Validate receipt</Text>}
      </TouchableOpacity>
    </ScrollView>
  );
}
